...
def get_hjorth_features() -> DataFrame:
    # Hjorth Mobility
    cohort_files = glob('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/**/hjorth_mobility.pickle', recursive=True)
    mobility = {}
    ...
    # Hjorth Complexity
    cohort_files = glob('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/**/hjorth_complexity.pickle', recursive=True)
    complexity = {}
    for file in cohort_files:
        with open(file, 'rb') as f:
            x = load(f)
    ...
    # Hjorth Activity
    cohort_files = glob('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/**/hjorth_activity.pickle', recursive=True)
    activity = {}
    for file in cohort_files:
        with open(file, 'rb') as f:
            x = load(f)
    ...
    # Save to CSV
    res.to_csv('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/1/cohort_hjorth.csv')

    return res


def get_pli_features() -> DataFrame:
    all_files = glob('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/**/pli_*.pickle', recursive=True)
    cohort_features = {}
    ...
    # Save?
    cohort_features.to_csv('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/1/cohort_pli.csv')

    return cohort_features

def get_spectral_features() -> DataFrame:
    all_files = glob('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/**/spectral.csv', recursive=True)
    ...
    cohort_features.to_csv('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/1/cohort_spectral.csv')
    return cohort_features

def read_spectral_features() -> DataFrame:
    return pd.read_csv('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/1/cohort_spectral.csv', index_col=0)

def read_hjorth_features() -> DataFrame:
    return pd.read_csv('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/1/cohort_hjorth.csv', index_col=0)

def read_pli_features() -> DataFrame:
    return pd.read_csv('/Volumes/MMIS-Saraiv/Datasets/KJPP/features/1/cohort_pli.csv', index_col=0)
...
def convert_feature_name_to_insight(name: str) -> str:
    if name.startswith('Hjorth'):
        type_and_channel = name.split(' ')[1]
        type, channel = type_and_channel.split(':')
        return f"{type.lower()}_{channel}"
...
def convert_feature_name_to_kjpp(name: str) -> str:
    if 'activity' in name:
        return f'Hjorth Activity:{name.split("_")[1]}'
    if 'complexity' in name:
        return f'Hjorth Complexity:{name.split("_")[1]}'
    if 'mobility' in name:
        return f'Hjorth Mobility:{name.split("_")[1]}'
    return name

...
class EventTestCase(unittest.TestCase):
    ...
    def test_index_biosignal_with_event_period(self):
        ...
        self.biosignal.associate(event)
        self.assertTrue(all(self.biosignal['seizure']._get_channel('x').samples == [3., 4., 5., 6.]))

    def test_index_biosignal_with_event_period_with_padding(self):
        event = Event('seizure', self.datetime2, self.datetime3)
        self.biosignal.associate(event)
        self.assertTrue(all(self.biosignal[timedelta(seconds=2):'seizure':timedelta(seconds=1)]._get_channel('x').samples == [1., 2., 3., 4., 5., 6., 7.]))
        self.assertTrue(all(self.biosignal[2:'seizure':1]._get_channel('x').samples == [1., 2., 3., 4., 5., 6., 7.]))
        self.assertTrue(all(self.biosignal['seizure':timedelta(seconds=1)]._get_channel('x').samples == [3., 4., 5., 6., 7.]))
        self.assertTrue(all(self.biosignal['seizure':1]._get_channel('x').samples == [3., 4., 5., 6., 7.]))
        self.assertTrue(all(self.biosignal[timedelta(seconds=1):'seizure']._get_channel('x').samples == [2., 3., 4., 5., 6.]))
        self.assertTrue(all(self.biosignal[1:'seizure']._get_channel('x').samples == [2., 3., 4., 5., 6.]))

        event = Event('seizure2', onset=self.datetime2)
        self.biosignal.associate(event)
        self.assertTrue(all(self.biosignal[timedelta(seconds=2):'seizure2':timedelta(seconds=1)]._get_channel('x').samples == [1., 2., 3.]))
        self.assertTrue(all(self.biosignal[2:'seizure2':1]._get_channel('x').samples == [1., 2., 3.]))
        self.assertTrue(all(self.biosignal['seizure2':timedelta(seconds=2)]._get_channel('x').samples == [3., 4.]))
        self.assertTrue(all(self.biosignal['seizure2':2]._get_channel('x').samples == [3., 4.]))
        self.assertTrue(all(self.biosignal[timedelta(seconds=2):'seizure2']._get_channel('x').samples == [1., 2.]))
        self.assertTrue(all(self.biosignal[2:'seizure2']._get_channel('x').samples == [1., 2.]))

        event = Event('seizure3', offset=self.datetime2)
        self.biosignal.associate(event)
        self.assertTrue(all(self.biosignal[timedelta(seconds=2):'seizure3':timedelta(seconds=1)]._get_channel('x').samples == [1., 2., 3.]))
        self.assertTrue(all(self.biosignal[2:'seizure3':1]._get_channel('x').samples == [1., 2., 3.]))
        self.assertTrue(all(self.biosignal['seizure3':timedelta(seconds=2)]._get_channel('x').samples == [3., 4.]))
        self.assertTrue(all(self.biosignal['seizure3':2]._get_channel('x').samples == [3., 4.]))
        self.assertTrue(all(self.biosignal[timedelta(seconds=2):'seizure3']._get_channel('x').samples == [1., 2.]))
        self.assertTrue(all(self.biosignal[2:'seizure3']._get_channel('x').samples == [1., 2.]))

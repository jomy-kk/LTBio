...
from .sources import __BiosignalSource as BS
...
from ..processing.noises.Noise import Noise
...
class Biosignal(ABC):
    ...
    def __init__(self,
                 timeseries: Dict[str | BodyLocation, timeseries.Timeseries] | str | Tuple[datetime],
                 source: BS.__subclasses__() = None,
                 patient: Patient = None,
                 acquisition_location: BodyLocation = None,
                 name: str = None,
                 **options):

        # Save BS, if given
        if source and not isinstance(source, tuple(BS.__subclasses__())):
            raise TypeError("Source must be a BS.")
        ...
        # Option 1: 'timeseries' is a string path -> Read samples from files
        if isinstance(timeseries, str):
            filepath = timeseries
            if source is None:
                raise ValueError("To read a Biosignal from files, specify a BS in 'source'.")
            else:
                # BS can give the samples (required) and many other optional metadata.
                # It's the BS that decides what it gives, depending on what it can get.
        ...
        # Option 2: 'timeseries' is a dictionary {chanel name: Timeseries} -> Save directly
        if isinstance(timeseries, dict):
            ...
            for key in timeseries.keys():
                if not isinstance(key, str) and not isinstance(key, BodyLocation):
                    raise TypeError("All keys in 'timeseries' must be strings or BodyLocation.")
            # Check if all values are Timeseries
            for ts in timeseries.values():
                if not isinstance(ts, Timeseries):
                    raise TypeError("All values in 'timeseries' must be Timeseries.")

            # Save dictionary of Timeseries
    ...
    def __copy__(self):
        return type(self)({ts: self.__timeseries[ts].__copy__() for ts in self.__timeseries}, self.__source, self.__patient,
                          self.__acquisition_location, str(self.__name))

    def _new(self, timeseries: Dict[str | BodyLocation, timeseries.Timeseries] | str | Tuple[datetime] = None,
             source: BS.__subclasses__() = None, patient: Patient = None, acquisition_location: BodyLocation = None, name: str = None,
        ...
        logging.disable(logging.WARNING)  # if outside the domain of every channel -> no problem; the Event will not be associated
        ...
        if len(signature(condition).parameters) > 1:
            ...
            for i in range(len(self._n_segments)):  # gives error if not all channles have the same domain
                ...
                for i in range(0, len(x[0]), window):
                    y = x[:, i: i + window]
                    evaluated += [y] * window
                intervals.append(timeseries.Timeseries._Timeseries__Segment._Segment__when(evaluated))
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, slice):
            ...
            if self.has_single_channel:  # one channel
                ...
                return self._new(timeseries={channel_name: channel[item]})  # FIXME: Why aren't events being updated here? (See below)
        ...
        if isinstance(item, tuple):
            ...
            # Time-related: Slices, Datetimes, Events, ...
                ...
                if isinstance(item[0], DateTimeRange):
                    item = sorted(item, key=lambda x: x.start_datetime)
                else:
                    item = sorted(item)

                return self._new({channel_name: channel[tuple(item)] for channel_name, channel in self})
    ...
    def patient(self) -> Patient:
        """Returns the associated patient, if any."""
        return self.__patient

    @property
    def patient_code(self):  # TODO: Delete method
        """deprecated: use .patient.code instead."""
        return self.__patient.code if self.__patient != None else 'n.d.'

    @property
    def patient_conditions(self) -> Set[MedicalCondition]:  # TODO: Delete method
        """deprecated: use .patient.conditions instead."""
    ...
    @property
    def source(self) -> BS:
        '''Returns the BS from where the data was read, or None if was not specified.'''
    ...
    @property
    def domain_timeline(self) -> Timeline:  # TODO: merge with domain
    ...
    def _to_dict(self) -> Dict[str | BodyLocation, timeseries.Timeseries]:  # TODO: Delete method
        return deepcopy(self.__timeseries)

    def _to_array(self) -> ndarray:
        """
        Converts Biosignal to a NumPy ndarray.
        :return: C x M x N array, where C is the number of channels, M the number of segments of each, and N their length.
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Biosignal):
            ...
            if (not self.has_single_channel or not other.has_single_channel) and self.channel_names != other.channel_names:
            ...
            return self._new(timeseries=res_timeseries, source=source, patient=patient, acquisition_location=acquisition_location,
                             name=name, events=events)
    ...
    def apply_and_new(self, operation, **kwargs):
        for channel in self.__timeseries.values():
            channel._apply_operation_and_new(operation, **kwargs)
    ...
    def withAdditiveNoise(cls, original, noise, name: str = None):
        ...
        events = events = set(original.__associated_events.values()).union(set(noise._Biosignal__associated_events.values())) if isinstance(
            noise, (Biosignal, timeseries.Timeseries)) else None
    ...
    def acquisition_scores(self):
        ...
        quality_score = self.quality_score(_onbody_duration=onbody_score * self.duration if onbody_score else self.duration)
    ...
    def onbody_score(self):
        if hasattr(self.source, 'onbody'):  # if the BS defines an 'onbody' method, then this score exists, it's computed and returned

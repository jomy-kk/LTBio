...
class INSIGHT(BiosignalSource):
    """This class represents the source of INSIGHT Database (in SET or MAT format) and includes methods to read and write
    ...
    def __init__(self, demographic_csv):
        super().__init__()
        INSIGHT.demographic_csv = demographic_csv
    ...
    def _timeseries(filepath, type=EEG, **options):
        """
        Reads the EDF file specified and returns a dictionary with one Timeseries per channel name.
        Args:
            filepath (str): Path to the EDF file
            type (Biosignal): Type of biosignal to extract. Only EEG allowed.
        """

        samples, initial_datetime, channel_names, sf = INSIGHT.__read_set_file(filepath, metadata=True)
        units = Volt(Multiplier.u)  # micro-volts

        # Get interruptions, if any
        boundaries_filepath = path.join(*path.split(filepath)[:-1], INSIGHT.BOUNDARIES_FILENAME)
        interruptions_exist = path.exists(boundaries_filepath)
        if interruptions_exist:
            interruptions_ixs = np.array(INSIGHT.__read_boundaries_file(boundaries_filepath))
        ...
        for ch in range(len(channel_names)):
            ...
            if interruptions_exist:
                ...
                for i in range(len(samples_by_segment)):
                    seg_initial_datetime = initial_datetime + timedelta(seconds=segments_initial_times[i] + i*INSIGHT.DELTA_BETWEEN_SEGMENTS)
                ...
                ts = Timeseries.withDiscontiguousSegments(samples_by_segment_with_time, sf, units,
                                                          name=f"{channel_names[ch]} equiv. of GSN-256")

            else:  # No interruptions
                ts = Timeseries(ch_samples, initial_datetime, sf, units, name=f"{channel_names[ch]} equiv. of GSN-256")
    ...
    def __find_sex_age(patient_code) -> tuple[Sex, int]:
        with open(INSIGHT.demographic_csv) as csv_file:
    ...
    def _patient(path, **options):
        ...
        sex, age = INSIGHT.__find_sex_age(patient_code)
        smc = SMC()

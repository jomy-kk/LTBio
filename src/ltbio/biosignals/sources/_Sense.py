...
class Sense(BiosignalSource):
    ...
    def __aux_date(header):
        ...
        return to_datetime(header[Sense.__KEY_TIME_IN_HEADER], ignoretz=True)
    ...
    def __get_defaults():
        ...
        if path.isfile(Sense.DEFAULTS_PATH) and access(Sense.DEFAULTS_PATH, R_OK):
            ...
            with open(Sense.DEFAULTS_PATH, 'r') as json_file:
                ...
                if Sense.__MODALITIES in json_string[Sense.DEVICE_ID]:
                    modalities = json_string[Sense.DEVICE_ID][Sense.__MODALITIES]
                else:
                    raise IOError(f"Key {Sense.__MODALITIES} is mandatory for each device default mapping.")

                # Get mapping of channel labels, if any
                if Sense.__CHANNEL_LABELS in json_string[Sense.DEVICE_ID]:
                    channel_labels = json_string[Sense.DEVICE_ID][Sense.__CHANNEL_LABELS]
                else:
                    channel_labels = None

                # Get body location, if any
                if Sense.__BODY_LOCATION in json_string[Sense.DEVICE_ID]:
                    body_location = json_string[Sense.DEVICE_ID][Sense.__BODY_LOCATION]
    ...
    def __read_file(file_path, type, channel_labels, modalities_available):
        ...
        sf = header[Sense.__KEY_HZ_IN_HEADER]
        ...
        if not Sense.BAD_FORMAT:
            ...
            for ix in mapping:
                label = mapping[ix]
                samples_of_interest[label] = all_samples[:, column_names.index(Sense.__ANALOGUE_LABELS_FORMAT.format(str(ix)))]
            ...
            for segment, start_index in zip(all_segments, start_indices):
                ...
                for ix in mapping:
                    label = mapping[ix]
                    samples_of_interest[label] = segment[:, column_names.index(Sense.__ANALOGUE_LABELS_FORMAT.format(str(ix)))]

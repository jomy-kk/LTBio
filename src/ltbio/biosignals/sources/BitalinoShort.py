...
class Bitalino(BiosignalSource):
    ...
    # @staticmethod
    def __read_bit(dirfile, sensors, initial_idx, end_idx, sensor_idx=[], sensor_names=[], device=''):
        """
        Reads one edf file
        Args:
            dirfile (str): contains the file path
        ...
        with open(dirfile) as fh:
            ...
            next(fh)
            # signal
            data = np.array([line.strip().split() for line in fh[initial_idx:end_idx]])
        ...
        if len(sensor_names) > 0:
            ...
            raise IOError(f"Sensor {sensors} were not found in this acquisition, please insert another")
    ...
    def _read(dir, type, startkey='A20', **options):
        ...
        new_dict = {}
        initial_idx = options['date1']
        end_idx = options['date2']
...
data = Bitalino._read(dir=path_, type=modalities.RESP, **options)
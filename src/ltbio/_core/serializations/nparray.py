...
def _from_array_timeseries(array: ndarray, start: datetime, sampling_frequency: float, units=None) -> Timeseries:
    ...
    # Separate the array into segments: save a dictionary with the initial datetime of each segment and the samples
    segments = {}
    current_segment = None
    for i, sample in enumerate(array):
        if np.isnan(sample):
            if current_segment is not None:
                segments[current_segment].append(sample)
        else:
            if current_segment is None:
                current_segment = start + timedelta(seconds=i / sampling_frequency)
                segments[current_segment] = [sample, ]
            else:
                segments[current_segment].append(sample)

    # Create the Timeseries
    return Timeseries.withDiscontiguousSegments(segments, sampling_frequency, units=units)

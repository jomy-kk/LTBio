...
class MITDB(BiosignalSource):
    ...
    @staticmethod
    def __read_dat(list, metadata=False):
    ...
    def _read(dir, type, **options):
        ...
        all_files = sorted(list(set([di.split('.')[0] for di in sorted(listdir(dir)) if di.endswith('dat')])))

        # run the dat read function for all files in list all_files
        channels, sfreq = MITDB.__read_dat(all_files[0], metadata=True)
